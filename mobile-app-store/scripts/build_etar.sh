...
# PLANETONYX Mobile - Etar source/build/sign/publish pipeline (MVP)
#
# Required env vars:
# - ETAR_VERSION (example: 1.0.53)
...
# - SOURCE_REPO (default: https://github.com/Etar-Group/Etar-Calendar.git)
# - SOURCE_REF (default: v${ETAR_VERSION})
...
python3 "${BASE_DIR}/scripts/publish_catalog.py" \
  ...
  --name "${APP_NAME}" \
  --channel "${CHANNEL}" \
  --version "${ETAR_VERSION}" \
  --artifact-url "${APPSTORE_BASE_URL}/artifacts/apps/${APP_ID}/${ETAR_VERSION}/${APP_ID}.apk" \
  --sha256 "${SHA256}" \
  --signature-url "${APPSTORE_BASE_URL}/artifacts/apps/${APP_ID}/${ETAR_VERSION}/${APP_ID}.apk.sig" \
