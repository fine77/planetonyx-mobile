...
APP_ID="fossify-contacts"
APP_NAME="Fossify Contacts"
LICENSE="GPL-3.0"
MAINTAINER_MODE="${MAINTAINER_MODE:-upstream}"
CHANNEL="${CHANNEL:-stable}"
FOSSIFY_CONTACTS_VERSION="${FOSSIFY_CONTACTS_VERSION:-}"
PRECHECK_ONLY="${PRECHECK_ONLY:-false}"
APPSTORE_BASE_URL="${APPSTORE_BASE_URL:-https://appstore.srv.planetonyx.net}"
SOURCE_REPO="${SOURCE_REPO:-https://github.com/FossifyOrg/Contacts.git}"
BUILD_CMD="${BUILD_CMD:-./gradlew --no-daemon assembleRelease}"

if [[ -z "${FOSSIFY_CONTACTS_VERSION}" ]]; then
  echo "ERROR: FOSSIFY_CONTACTS_VERSION is required (example: 1.6.0)"
  exit 1
fi

SOURCE_REF="${SOURCE_REF:-${FOSSIFY_CONTACTS_VERSION}}"
SRC_DIR="${BASE_DIR}/sources/${APP_ID}"
PATCH_DIR="${BASE_DIR}/patches/${APP_ID}"
BUILD_DIR="${BASE_DIR}/build/${APP_ID}"
DIST_DIR="${BASE_DIR}/dist/${APP_ID}/${FOSSIFY_CONTACTS_VERSION}"
...
if [[ -n "${DETACHED_SIGN_KEY_PATH:-}" && -f "${DETACHED_SIGN_KEY_PATH}" ]] && command -v openssl >/dev/null 2>&1; then
  ...
  printf "SIGNATURE_PLACEHOLDER_FOR_%s_%s\n" "${APP_ID}" "${FOSSIFY_CONTACTS_VERSION}" > "${SIG_FILE}"
fi
...
python3 "${BASE_DIR}/scripts/publish_catalog.py" \
  ...
  --name "${APP_NAME}" \
  --channel "${CHANNEL}" \
  --version "${FOSSIFY_CONTACTS_VERSION}" \
  --artifact-url "${APPSTORE_BASE_URL}/artifacts/apps/${APP_ID}/${FOSSIFY_CONTACTS_VERSION}/${APP_ID}.apk" \
  --sha256 "${SHA256}" \
  --signature-url "${APPSTORE_BASE_URL}/artifacts/apps/${APP_ID}/${FOSSIFY_CONTACTS_VERSION}/${APP_ID}.apk.sig" \
