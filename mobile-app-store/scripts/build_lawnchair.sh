...
# - SOURCE_REPO (default: https://github.com/LawnchairLauncher/lawnchair.git)
# - SOURCE_REF (default: v${LAWNCHAIR_VERSION})
...
python3 "${BASE_DIR}/scripts/publish_catalog.py" \
  ...
  --name "${APP_NAME}" \
  --channel "${CHANNEL}" \
  --version "${LAWNCHAIR_VERSION}" \
  --artifact-url "${APPSTORE_BASE_URL}/artifacts/apps/${APP_ID}/${LAWNCHAIR_VERSION}/${APP_ID}.apk" \
  --sha256 "${SHA256}" \
  --signature-url "${APPSTORE_BASE_URL}/artifacts/apps/${APP_ID}/${LAWNCHAIR_VERSION}/${APP_ID}.apk.sig" \
