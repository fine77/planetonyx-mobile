...
# PLANETONYX Mobile - Syncthing Android source/build/sign/publish pipeline (MVP)
#
# Required env vars:
# - SYNCTHING_ANDROID_VERSION (example: 1.2.0.8)
...
APP_ID="syncthing-android"
APP_NAME="Syncthing Android"
LICENSE="MPL-2.0"
...
if [[ -z "${SYNCTHING_ANDROID_VERSION}" ]]; then
  echo "ERROR: SYNCTHING_ANDROID_VERSION is required (example: 1.2.0.8)"
  exit 1
fi

SOURCE_REF="${SOURCE_REF:-v${SYNCTHING_ANDROID_VERSION}}"
SRC_DIR="${BASE_DIR}/sources/${APP_ID}"
PATCH_DIR="${BASE_DIR}/patches/${APP_ID}"
BUILD_DIR="${BASE_DIR}/build/${APP_ID}"
DIST_DIR="${BASE_DIR}/dist/${APP_ID}/${SYNCTHING_ANDROID_VERSION}"
...
if [[ -n "${DETACHED_SIGN_KEY_PATH:-}" && -f "${DETACHED_SIGN_KEY_PATH}" ]] && command -v openssl >/dev/null 2>&1; then
  ...
  printf "SIGNATURE_PLACEHOLDER_FOR_%s_%s\n" "${APP_ID}" "${SYNCTHING_ANDROID_VERSION}" > "${SIG_FILE}"
fi
...
python3 "${BASE_DIR}/scripts/publish_catalog.py" \
  ...
  --name "${APP_NAME}" \
  --channel "${CHANNEL}" \
  --version "${SYNCTHING_ANDROID_VERSION}" \
  --artifact-url "${APPSTORE_BASE_URL}/artifacts/apps/${APP_ID}/${SYNCTHING_ANDROID_VERSION}/${APP_ID}.apk" \
  --sha256 "${SHA256}" \
  --signature-url "${APPSTORE_BASE_URL}/artifacts/apps/${APP_ID}/${SYNCTHING_ANDROID_VERSION}/${APP_ID}.apk.sig" \
