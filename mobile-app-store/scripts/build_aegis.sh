...
# PLANETONYX Mobile - Aegis source/build/sign/publish pipeline (MVP)
#
# Required env vars:
# - AEGIS_VERSION (example: 3.4.2)
...
# - SOURCE_REPO (default: https://github.com/beemdevelopment/Aegis.git)
# - SOURCE_REF (default: v${AEGIS_VERSION})
...
SOURCE_REPO="${SOURCE_REPO:-https://github.com/beemdevelopment/Aegis.git}"
BUILD_CMD="${BUILD_CMD:-./gradlew --no-daemon assembleRelease}"

if [[ -z "${AEGIS_VERSION}" ]]; then
  echo "ERROR: AEGIS_VERSION is required (example: 3.4.2)"
  exit 1
fi

SOURCE_REF="${SOURCE_REF:-v${AEGIS_VERSION}}"
SRC_DIR="${BASE_DIR}/sources/${APP_ID}"
PATCH_DIR="${BASE_DIR}/patches/${APP_ID}"
BUILD_DIR="${BASE_DIR}/build/${APP_ID}"
DIST_DIR="${BASE_DIR}/dist/${APP_ID}/${AEGIS_VERSION}"
...
python3 "${BASE_DIR}/scripts/publish_catalog.py" \
  ...
  --artifact-url "${APPSTORE_BASE_URL}/artifacts/apps/${APP_ID}/${AEGIS_VERSION}/${APP_ID}.apk" \
  --sha256 "${SHA256}" \
  --signature-url "${APPSTORE_BASE_URL}/artifacts/apps/${APP_ID}/${AEGIS_VERSION}/${APP_ID}.apk.sig" \
