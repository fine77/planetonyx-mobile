...
TASKSORG_VERSION="${TASKSORG_VERSION:-}"
PRECHECK_ONLY="${PRECHECK_ONLY:-false}"
APPSTORE_BASE_URL="${APPSTORE_BASE_URL:-https://appstore.srv.planetonyx.net}"
SOURCE_REPO="${SOURCE_REPO:-https://github.com/tasks/tasks.git}"
BUILD_CMD="${BUILD_CMD:-./gradlew --no-daemon assembleRelease}"

if [[ -z "${TASKSORG_VERSION}" ]]; then
  echo "ERROR: TASKSORG_VERSION is required (example: 15.1)"
  exit 1
fi

SOURCE_REF="${SOURCE_REF:-${TASKSORG_VERSION}}"
SRC_DIR="${BASE_DIR}/sources/${APP_ID}"
PATCH_DIR="${BASE_DIR}/patches/${APP_ID}"
BUILD_DIR="${BASE_DIR}/build/${APP_ID}"
DIST_DIR="${BASE_DIR}/dist/${APP_ID}/${TASKSORG_VERSION}"
...
python3 "${BASE_DIR}/scripts/publish_catalog.py" \
  ...
  --name "${APP_NAME}" \
  --channel "${CHANNEL}" \
  --version "${TASKSORG_VERSION}" \
  --artifact-url "${APPSTORE_BASE_URL}/artifacts/apps/${APP_ID}/${TASKSORG_VERSION}/${APP_ID}.apk" \
  --sha256 "${SHA256}" \
  --signature-url "${APPSTORE_BASE_URL}/artifacts/apps/${APP_ID}/${TASKSORG_VERSION}/${APP_ID}.apk.sig" \
