...
# PLANETONYX Mobile - K-9 Mail source/build/sign/publish pipeline (MVP)
#
# Required env vars:
# - K9_VERSION (example: 17.0)
...
# - SOURCE_REPO (default: https://github.com/thunderbird/thunderbird-android.git)
# - SOURCE_REF (default: THUNDERBIRD_${K9_VERSION//./_})
...
K9_VERSION="${K9_VERSION:-}"
PRECHECK_ONLY="${PRECHECK_ONLY:-false}"
APPSTORE_BASE_URL="${APPSTORE_BASE_URL:-https://appstore.srv.planetonyx.net}"
SOURCE_REPO="${SOURCE_REPO:-https://github.com/thunderbird/thunderbird-android.git}"
BUILD_CMD="${BUILD_CMD:-./gradlew --no-daemon assembleRelease}"

if [[ -z "${K9_VERSION}" ]]; then
  echo "ERROR: K9_VERSION is required (example: 17.0)"
  exit 1
fi

DEFAULT_SOURCE_REF="THUNDERBIRD_${K9_VERSION//./_}"
...
DIST_DIR="${BASE_DIR}/dist/${APP_ID}/${K9_VERSION}"
...
if ! git -C "${SRC_DIR}" rev-parse -q --verify "${SOURCE_REF}^{commit}" >/dev/null; then
  ...
  git -C "${SRC_DIR}" tag -l 'THUNDERBIRD_*' --sort=-v:refname | head -n 12 | sed 's/^/  - /'
  exit 1
fi
...
python3 "${BASE_DIR}/scripts/publish_catalog.py" \
  ...
  --channel "${CHANNEL}" \
  --version "${K9_VERSION}" \
  --artifact-url "${APPSTORE_BASE_URL}/artifacts/apps/${APP_ID}/${K9_VERSION}/${APP_ID}.apk" \
  --sha256 "${SHA256}" \
  --signature-url "${APPSTORE_BASE_URL}/artifacts/apps/${APP_ID}/${K9_VERSION}/${APP_ID}.apk.sig" \
